  ...
  return (
    <div style={{minHeight:"100vh",background:"#040912"}}>
      <nav data-app-chrome style={{background:"#1E3A5F",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid rgba(0,180,216,0.15)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>🎉</span><span style={{fontWeight:800,color:"#00B4D8",fontSize:15}}>Javari Events</span></div>
      ...
      <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"36px 24px 28px",textAlign:"center"}}>
        ...
        <h1 style={{fontSize:"clamp(20px,3.5vw,38px)",fontWeight:900,color:"#fff",margin:"0 0 8px"}}>Javari Events</h1>
        <p style={{color:"rgba(255,255,255,0.7)",fontSize:14,margin:0}}>Plan it. Promote it. Perfect it.</p>
      </section>
      <section style={{maxWidth:900,margin:"0 auto",padding:"20px 16px 0"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:20}}>
        <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.1)",borderRadius:12,padding:16}}><div style={{fontSize:24,marginBottom:6}}>📬</div><div style={{fontWeight:700,color:"#e2e8f0",fontSize:13,marginBottom:3}}>Invitation Writer</div><div style={{fontSize:11,color:"#6B7280"}}>Digital and print invites</div></div>
        <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.1)",borderRadius:12,padding:16}}><div style={{fontSize:24,marginBottom:6}}>📣</div><div style={{fontWeight:700,color:"#e2e8f0",fontSize:13,marginBottom:3}}>Event Promotion</div><div style={{fontSize:11,color:"#6B7280"}}>Social and email copy</div></div>
        <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.1)",borderRadius:12,padding:16}}><div style={{fontSize:24,marginBottom:6}}>📋</div><div style={{fontWeight:700,color:"#e2e8f0",fontSize:13,marginBottom:3}}>Day-Of Timeline</div><div style={{fontSize:11,color:"#6B7280"}}>Minute-by-minute schedule</div></div>
        <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.1)",borderRadius:12,padding:16}}><div style={{fontSize:24,marginBottom:6}}>📧</div><div style={{fontWeight:700,color:"#e2e8f0",fontSize:13,marginBottom:3}}>Vendor Comms</div><div style={{fontSize:11,color:"#6B7280"}}>Professional vendor emails</div></div>
      ...
      <section style={{maxWidth:900,margin:"0 auto",padding:"0 16px 48px"}}>
        <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.12)",borderRadius:14,padding:18}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
            <button onClick={() => setInput("Write an invitation for a corporate holiday party for 150 guests")} style={{background:"rgba(0,180,216,0.06)",border:"1px solid rgba(0,180,216,0.15)",borderRadius:6,padding:"4px 10px",fontSize:11,color:"#9CA3AF",cursor:"pointer",fontFamily:"system-ui"}}>Write an invitation for a corporate holiday party fo…</button>
            <button onClick={() => setInput("Create a day-of timeline for a 200-person outdoor wedding")} style={{background:"rgba(0,180,216,0.06)",border:"1px solid rgba(0,180,216,0.15)",borderRadius:6,padding:"4px 10px",fontSize:11,color:"#9CA3AF",cursor:"pointer",fontFamily:"system-ui"}}>Create a day-of timeline for a 200-person outdoor we…</button>
            <button onClick={() => setInput("Write a vendor follow-up email for a catering company")} style={{background:"rgba(0,180,216,0.06)",border:"1px solid rgba(0,180,216,0.15)",borderRadius:6,padding:"4px 10px",fontSize:11,color:"#9CA3AF",cursor:"pointer",fontFamily:"system-ui"}}>Write a vendor follow-up email for a catering compan…</button>
                      </div>
          <div style={{display:"flex",gap:8}}>
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&ask()}
              placeholder="Ask Javari Events AI anything..."
